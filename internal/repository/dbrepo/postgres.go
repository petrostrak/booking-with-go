...
// InsertRoomRestriction insterts a room restriction into the db
...
// UpdateUser updates the user to the DB
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	row := m.DB.QueryRowContext(ctx, "select id, password from userds where email = $1", email)
	if err := row.Scan(&id, &hashedPassword); err != nil {
		return id, "", err
	}

	if err := bcrypt.CompareHashAndPassword([]byte(hashedPassword), []byte(testPassword)); err == bcrypt.ErrMismatchedHashAndPassword {
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_data, r.end_date, r.room_id, r.created_at, r.updated_at, r.processed, rm.id, rm.room_name 
			  from reservations r
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	query := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_data, r.end_date, r.room_id, r.created_at, r.updated_at, rm.id, rm.room_name 
			  from reservations r
			  left join rooms rm on (r.room_id = rm.id)
			  where processed = 0
			  order by r.start_date asc`
...
func (m *postgresDBRepo) GetReservationByID(id int) (models.Reservation, error) {
	...
	query := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, r.created_at, r.processed, rm.id, rm.room_name
...
// UpdateReservation updates the user to the DB
...
func (m *postgresDBRepo) UpdateProcessedForReservation(id, process int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `update reservations set processed = $1 where id = $2`

	if _, err := m.DB.ExecContext(ctx, query, process, id); err != nil {
		return err
	}
...
func (m *postgresDBRepo) GetRestictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
