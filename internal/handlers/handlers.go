...
import (
	...
	"github.com/petrostrak/booking-with-go/internal/forms"
	...
	"github.com/petrostrak/booking-with-go/internal/repository/dbrepo"
)
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	htmlMsg := fmt.Sprintf(`
		...
		Dear %s: <br>
		This mail confirms your reservation from %s to %s.
...
// AvailabilityJSON handles request for availability and sends json respond
...
// PostShowLoginhandles logging the user in
...
func (m *Repository) Logout(w http.ResponseWriter, r *http.Request) {
	if err := m.App.Session.Destroy(r.Context()); err != nil {
		m.App.ErrorLog.Println("something went wrong while logging out")
	}
	if err := m.App.Session.RenewToken(r.Context()); err != nil {
		m.App.ErrorLog.Println("couldn't renew token")
	}
...
// AdminAllReservations shos all reservations in admin tool
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		restrictions, err := m.DB.GetRestictionsForRoomByDate(x.ID, firstOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
			return
		}
			...
			if val, ok := curMap[name]; ok {
				if val > 0 {
					if !form.Has(fmt.Sprintf("remove_block_%d_%s", x.ID, name), r) {
						log.Println("would delete blocl", value)
					}
				}
			}
		}
